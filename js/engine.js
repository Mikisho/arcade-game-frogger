...
var Engine = (function(global) {
 ...
 function checkCollisions() {
  ...
  allEnemies.forEach(function(enemy) {
   for (var i = enemy.x; i <= enemy.x + CountY; i++) {
    if (i >= player.x + colSpace && i <= player.x + CountY - colSpace) {
     for (var j = enemy.y - CountY; j <= enemy.y; j++) {
      if (j >= player.y - CountY + colSpace && j <= player.y - colSpace) {
       player.reset(0);
       reset();
      }
     }
  ...
  canvasPrizes.forEach(function(canvasPrize) {
   if (canvasPrize.sprite == "images/Rock.png") {
    for (var i = canvasPrize.x; i <= canvasPrize.x + CountY; i++) {
     if (i >= player.x + colSpace && i <= player.x + CountY - colSpace) {
      for (var j = canvasPrize.y - CountY; j <= canvasPrize.y; j++) {
       if (j >= player.y - CountY + colSpace && j <= player.y - colSpace) {
    ...
    // if player hits a Prize item, then increase score and remove Prize from canvas
   } else if ((player.x == canvasPrize.x) && (player.y - PrizeYPos == canvasPrize.y)) {
    player.collect(canvasPrize.points);
  ...
  */
 function reset() {
  // when reset game, place new Prizes on canvas
  placePrizesOnCanvas();
 }
